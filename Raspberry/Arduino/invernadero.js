...
var ports = [
  ...
  { id: 'B', port: '/dev/ttyUSB1' } //Proximidad
]
...
var sensorCOM3 = '/dev/ttyUSB' // sensor humedad
...
const agent = new ModAgent({interval: sendDatos,
  agentID: 
  agentID })      
...
board.on('ready', function () {
  ...
  if (swAutoSave == 3) {
    ...
    console.log(
      '\x1b[32m',
      `${swAutoSave}/3 Conexion a internet exitosa, trabajando con el servidor.`
    )
    console.log('\x1b[37m', '')
  } else {
    ...
    console.log(
      '\033[33m',
      `Conexion a internet fallida, trabajando con archivos de respaldo`
    )
    console.log('\x1b[37m', '')
  }
  PrimerRiego()
      ...
      for (let i = 0; i < data.length; i++) {
        if (data.charCodeAt(i) == 32 || data.charCodeAt(i) == 13) {
          ...
          if (!isNaN(SensoresDTH2[j])) {
            ...
            if (SensoresDTH2[j] != 0) {
              temp[
                j + totalPinesMega + SensoresDTH.length + SensoresDS.length + 3
              ] = SensoresDTH2[j]
            }
          }
          a = ''
  ...
  for (let i = 54; i < estado.length; i++) {
    if (estado[i] == 1 && i != 63 && i != 61 && i != 62) {
      if (clasePin[i] == 1) {
        ...
        console.log('este son agent :', descripcion[i])
  ...
  this.each(function (board) {
    if (board.id === 'A') {
      ...
      setInterval(function () {
        ...
        if (
          pulsesFlow[55] > pulsacionesFlow &&
          pulsesFlow[63] > pulsacionesFlow
        ) {
          temp[55] = 1
          //temp[63]=1
        } else {
        ...
        if (
          pulsesFlow[60] > pulsacionesFlow &&
          pulsesFlow[61] > pulsacionesFlow &&
          pulsesFlow[62] > pulsacionesFlow
        ) {
          temp[60] = 1
          //temp[61]=1
          //temp[62]=1
        ...
        if (ssww2 == 0) {
          ...
          console.log(
            Hora +
              ...
              ':' +
              Segundos +
              ' Actualiza la base de datos, con los finales de carrera'
          )
            ...
            if (estado[i] == 1 && descripcion[i].indexOf('temp') >= 0) {
                ...
                if (temp[i] >= tempMedia) {
                  ...
                  console.log(
                    ...
                    Hora +
                      ...
                      ':' +
                      Segundos +
                      'Funcion apertura puertas y ventanas para ' +
                      descripcion[i]
                  )
                  ...
                  if (temp[i] < tempMedia - 10) {
                    ...
                    console.log(
                      ...
                      Hora +
                        ...
                        ':' +
                        Segundos +
                        ' Funcion cerrar puertas y ventanas ' +
                        descripcion[i]
                    )
              ...
              if (descansoVentilador[i] && swPuertas[i]) {
                if (temp[i] >= tempMaxima) {
                  ...
                  console.log(
                    '\x1b[32m',
                    Hora + ':' + Minuto + ':' + Segundos + ' Funcion ventilador'
                  )
      ...
      this.loop(60000, () => {
        ...
        if (date2.getHours() == 7 && date2.getMinutes() == 30) {
          notificacion(
            ...
            'Notificacion  y reinicio de sistema en correcto funcionamiento a horas 7:30 a.m.'
          )
          setTimeout(function () {
            restartInvernadero()
          }, 1000)
                ...
                if (estado[i] == 1) {
                  if (clasePin[i] == 1) {
                    if (
                      descripcion[i]
                        .toLowerCase()
                        .indexOf(idBomba.descripcionPin) >= 0
                    ) {
        ...
        for (let i = totalPinesMega; i < estado.length; i++) {
          if (estado[i] == 1) {
            if (clasePin[i] == 1) {
              if (
                descripcion[i].toLowerCase().indexOf('sensor temperatura') >= 0
              ) {
                if (temp[i] > 40) {
                  notificacion(
                    `Precaucion ${descripcion[i]}`,
                    `${descripcion[i]} a mas de 40°, , Revisar el sistema...!!!`
                  )
                  break
                }
              }
        ...
        if (payload.agent.uuid == agentID) {
              ...
              for (let i = 0; i < estado.length; i++) {
                if (payload.actuador.type == nroPin[i]) {
                  if (payload.actuador.value == 1) {
                    ...
                    notificacion(
                      `funcionamiento`,
                      `${descripcion[i]} ON, ${accion}`
                    )
                  } else {
                    //console.log("si")
                    notificacion(
                      `funcionamiento`,
                      `${descripcion[i]} OFF, ${accion}`
                    )
                  }
                }
              }
                ...
                for (let j = 14; j < idPin.length; j++) {
                  if (payload.actuador.type + '' == nroPin[j] + '') {
                    for (let k = 0; k < idPin.length; k++) {
                      ...
                      if (
                        depende[j] == depende[k] &&
                        descripcion[k].indexOf('finalOn') < 0 &&
                        descripcion[k].indexOf('finalOff') < 0
                      ) {
          ...
          if (topic == 'actualizacion') {
            /*conf()
              ...
              */
            notificacion(
              'Reinicio del Invernadero por Usuario',
              ' Reiniciando Invernadero a peticion de usuario'
            )
            setTimeout(function () {
              restartInvernadero()
            }, 1000)
          ...
          if (topic == 'eliminarRiego') {
            ...
            for (let i = 0; i < hIni.length; i++) {
              if (
                hIni[i] == payload.hora &&
                hDur[i] == payload.duracion &&
                hPinId[i] == parseInt(payload.bomba)
              ) {
                console.log(hIni[i])
                console.log(hDur[i])
  ...
  this.each(function (board) {
    if (board.id === 'B') {
      ...
      proximity1.on('change', function () {
        ...
        temp[88] = (
          ((proximidadTanque1 - proximidadDato1) / 100) *
          100
        ).toFixed(2)
        //temp[81]=this.cm
      })
      ...
      proximity2.on('change', function () {
        ...
        temp[89] = (
          ((proximidadTanque2 - proximidadDato2) / 100) *
          100
        ).toFixed(2)
        //temp[81]=this.cm
      })
      ...
      proximity3.on('change', function () {
        ...
        temp[90] = (
          ((proximidadTanque3 - proximidadDato3) / 100) *
          100
        ).toFixed(2)
        //temp[81]=this.cm
      })
...
async function encenderVentilador(idP, nroP, pos) {
  ...
  for (let j = 0; j < idPin.length; j++) {
    if (
      sensorId[j] == idP &&
      descripcion[j].toLowerCase().indexOf('ventilador') >= 0
    ) {
      var HoraYFecha = new Date()
      var Hora = HoraYFecha.getHours()
      var Minuto = HoraYFecha.getMinutes()
              ...
              setTimeout(function () {
                ...
                console.log(
                  ...
                  Hora +
                    ...
                    ':' +
                    Segundos +
                    ' Error, final de carrera no detectado, deteniendo motor : ' +
                    descripcion[j]
                )
                var payload = `{"agent":{"uuid":"${agentID}"},"actuador":{"type":${nroPin[j]},"value":0},"timestamp":1617522290000}`
                client.publish('actuador', payload)
            ...
            setTimeout(function () {
              ...
              setTimeout(function () {
                ...
                if (temp[j2] < 200) {
                  console.log(
                    '\x1b[31m',
                    'error no termino de cerrar : ' +
                      descripcion[j].substring(0, descripcion[j].length)
                  )
                }
              }, TiempoFuncionMotor)
            }, TiempoApertura)
...
async function PrimerRiego() {
  ...
  if (date2.getHours() == 7 && date2.getMinutes() == 30) {
    notificacion(
      'Sistema en Funcionamiento',
      'Notificacion de sistema en correcto funcionamiento a horas 7:30 a.m.'
    )
  }
      ...
      if (H == date.getHours() && M == date.getMinutes()) {
        ...
        for (let i = 0; i < totalPinesMega; i++) {
          if (estado[i] == 1) {
            if (clasePin[i] == 1) {
              if (
                descripcion[i].toLowerCase().indexOf(idBomba.descripcionPin) >=
                0
              ) {
                bombas.push(idBomba.descripcionPin)
                pos.push(i)
...
async function commando() {
  ...
  var ping = parseInt(
    a.substring(a.indexOf('received') - 2, a.indexOf('received'))
  )
  ...
  if (ping == 5) {
    ...
    if (shCont2 == 1) {
      shCont2 = 0
      notificacion(
        'Reestableciendo conexion a Internet',
        ' Reiniciando Invernadero'
      )
      setTimeout(function () {
        restartInvernadero()
      }, 1000)
...
async function CheckSensores() {
  ...
  if (checkcont2 == checkMin) {
    ...
    notificacion(
      'Error Sensores de Temperatura',
      'Los datos de los sensores de temperatura no se actualizaron por 10 minutos, el sistema se reiniciara'
    )
    setTimeout(function () {
      restartInvernadero()
    }, 1000)
