...
/*  CODIGO CON FINALES DE CARRERA  */
 ...
 /* CODIGO SIN FINALES DE CARRERA */
    ...
    var min= e.target.elements[5].value;
    var sec= e.target.elements[6].value;
    if(min<10)
    {
      min='0'+min;
    }
   ...
   this.pinService.Activate(localStorage.getItem('idControlador'),this.pinD1,this.Descripcion,'-1',this.dependence,modelo,marca,'4',Tmotor).subscribe(
      response =>{
        this.result=response;
          ...
          //final de carreraa 
          this.pinService.Activate(localStorage.getItem('idControlador'),this.pinD2,this.Descripcion+'Off',this.result['id'],'-1',modelo,marca,'3',Tmotor).subscribe(
          response =>{
          },
          error =>{
          }
