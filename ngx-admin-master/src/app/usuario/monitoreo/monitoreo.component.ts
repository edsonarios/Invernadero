                        ...
                        response =>{
                          ...
                          if (Array.isArray(response)) {
                            response.forEach(m => {
                              ...
                              if (m.type.substring(7,14)=="tempera"||m.type.substring(7,14)=="humedad") {
...
ngOnInit(){
  ...
  this._controlService.showControlador(localStorage.getItem('user_inv_id')).subscribe(
      response =>{
                          ...
                          if (Array.isArray(response)) {
                            response.forEach(m => {
                              ...
                              if((m.type.substring(13)).toLowerCase()==(cadena).toLowerCase()){
                                ...
                                this.FlujoMetricsUuid.push(aux);
                              }
 ...
 ngAfterViewInit(): void {
  ...
  openWindow(nombre,SensorUuid) {
    this.windowService.open(
      ...
      { title: nombre, context: { Type: nombre, Uuid: SensorUuid },hasBackdrop: true, closeOnEsc: false },
    );
